...
// ─── Helper: post Facebook live comment ───────────────────────────────────────
...
async function postFacebookComment(liveVideoId: string, message: string) {
  ...
  try {
    ...
    else console.log('FB comment posted:', message);
  } catch (err: any) {
    console.error('Facebook post error:', err.message);
  }
}

// ─── Helper: build score text ─────────────────────────────────────────────────
...
async function startServer() {
    ...
    try {
      ...
      res.send(`
        <html><body>
          ...
          <p style="font-family:sans-serif;text-align:center;margin-top:40px;color:#16a34a">
  ...
  app.post('/api/facebook/update', async (req, res) => {
    const { live_id, text } = req.body;
    await postFacebookComment(live_id, text);
  ...
  app.get('/api/teams', (req, res) => {
    res.json(db.prepare('SELECT * FROM teams ORDER BY id DESC').all());
  });
  ...
  app.post('/api/matches/:id/toss', (req, res) => {
    ...
    const batting_team_id = toss_decision === 'bat'
      ? toss_winner_id
      : (match.team1_id === toss_winner_id ? match.team2_id : match.team1_id);
    db.prepare('INSERT INTO innings (match_id, batting_team_id, innings_number) VALUES (?, ?, 1)')
      .run(req.params.id, batting_team_id);
    res.json({ success: true });
  });

  app.post('/api/matches/:id/youtube', (req, res) => {
    const { broadcast_id } = req.body;
    db.prepare('UPDATE matches SET youtube_broadcast_id = ? WHERE id = ?').run(broadcast_id, req.params.id);
    res.json({ success: true });
  });

  app.post('/api/matches/:id/complete', (req, res) => {
    const { winner_id, result_note } = req.body;
    db.prepare("UPDATE matches SET winner_id = ?, result_note = ?, status = 'completed' WHERE id = ?")
      .run(winner_id, result_note, req.params.id);
    res.json({ success: true });
  });
  ...
  app.post('/api/innings', (req, res) => {
    ...
    const result = db.prepare('INSERT INTO innings (match_id, batting_team_id, innings_number) VALUES (?, ?, ?)')
      .run(match_id, batting_team_id, innings_number);
    res.json({ id: result.lastInsertRowid });
  });
  ...
  app.post('/api/balls', async (req, res) => {
    try {
      ...
      // Async: update YouTube title & Facebook comment
      ...
      if (innings) {
        ...
        if (match) {
          ...
          if (match.youtube_broadcast_id && req.cookies.google_tokens) {
            ...
            updateYouTubeTitle(tokens, match.youtube_broadcast_id, scoreText).catch(console.error);
          }

          // Facebook — post every over completion or wicket
          if (match.facebook_live_id && (is_wicket || (is_valid && ball_number === 6))) {
            const fbMsg = is_wicket
              ? `🔴 WICKET! ${scoreText}`
              : `📊 Over ${over_number + 1} complete: ${scoreText}`;
  ...
  app.get('/api/matches/:id/scorecard', (req, res) => {
    ...
    const scorecard = innings.map(inn => {
      ...
      const bowlingStats = db.prepare(`
        SELECT p.name,
          COUNT(CASE WHEN b.extras_type IS NULL OR b.extras_type NOT IN ('wide','no_ball') THEN 1 END) as balls,
          SUM(b.runs_batter + b.extras_runs) as runs,
      ...
      return {
        ...
        battingStats,
        bowlingStats,
        extras: extras || { total: 0, wides: 0, no_balls: 0, byes: 0, leg_byes: 0 }
      };
    });
    res.json(scorecard);
  });
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🏏 Cricket Scorer Pro running on http://localhost:${PORT}\n`);
  });
}
